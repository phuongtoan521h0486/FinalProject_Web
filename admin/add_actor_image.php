...
if ($_SERVER['REQUEST_METHOD'] !== 'POST')
  ...
  if (!isset($_FILES["fileupload_actor"]))
  {
      die;
  }

  if ($_FILES["fileupload_actor"]['error'] != 0)
  {
    die;
  }
  ...
  $target_file   = $target_dir . basename($_FILES["fileupload_actor"]["name"]);
  ...
  if(isset($_POST["submit_actor"])) {
      $check = getimagesize($_FILES["fileupload_actor"]["tmp_name"]);
      if($check !== false)
      {
          $allowUpload = true;
      }
  ...
  if ($_FILES["fileupload_actor"]["size"] > $maxfilesize)
  {
      $allowUpload = false;
  }
  ...
  if ($allowUpload)
  {
      move_uploaded_file($_FILES["fileupload_actor"]["tmp_name"], $target_file);
  }
  rename($target_file, $target_dir.$_POST['new_actor_name'] . "." .pathinfo($target_file, PATHINFO_EXTENSION));
?>